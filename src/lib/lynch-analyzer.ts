...
function computeInsiderTrading(
  ...
  if (asOf != null && basis != null && Number.isFinite(basis)) {
    ...
    // asOf가 기록돼 있으면 "수집은 완료했으나 최근 6개월 내 소유보고 변동이 없는" 상태
    // (대형·지주·금융사 등 내부자 거래가 드문 종목). asOf가 null이면 아직 수집되지 않았거나
    // 수집에 실패한 상태. 두 경우를 코멘트로 구분해 '신호 공백'을 정직하게 드러낸다.
    comment = asOf != null
      ? "최근 6개월 임원·주요주주 소유보고 변동 없음(수집 완료)."
      : "임원·주요주주 소유보고 미수집.";
  }
